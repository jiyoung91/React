...
const dataList =[
    {id:1,name:'chk1',text:'연령(만 14세이상) 확인 필수',isChk:false},
    {id:2,name:'chk2',text:'연령(만 14세이상) 확인 필수',isChk:false},
        ...
        <div>
            <p>
                <input type="checkbox" name="all" onChange={changeInput}
                 checked={
                    data.filter(item=> item.isChk === false).length === 0  
                 }/>
                <label>약관동의</label>
            </p>
            <hr />
            {
                data.map(item=><p key={item.id}>
                    <input type="checkbox" name={item.name} checked={item.isChk} 
